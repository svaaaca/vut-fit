...
-- | trimFilterId is used in buildFilterSpec to strip leading/trailing whitespace from filter criteria
...
--
-- This function converts lists of filter strings from the command-line arguments
-- into structured 'FilterCriterion' values and assembles them into a 'FilterSpec'.
--
-- Each input string is trimmed of leading/trailing whitespace and then wrapped
-- in the appropriate 'FilterCriterion' constructor:
--
-- * strings from @--include@ / @-i@ become 'ByAny' criteria,
-- * strings from @--ic@ become 'ByCategory' criteria,
-- * strings from @--it@ become 'ByTag' criteria,
-- * exclude strings follow the same pattern.
--
-- The resulting filter spec combines all include criteria and all exclude criteria
-- into separate lists for use by the filtering algorithm.
buildFilterSpec :: [String] -> [String] -> [String] -> [String] -> [String] -> [String] -> FilterSpec
buildFilterSpec includeAny excludeAny includeCategories includeTags excludeCategories excludeTags =
  -- transform each list of strings into the appropriate FilterCriterion type,
  -- trimming whitespace and wrapping each string
  let includeByAny = map (ByAny . trimFilterId) includeAny
      includeByCategory = map (ByCategory . trimFilterId) includeCategories
      includeByTag = map (ByTag . trimFilterId) includeTags
      excludeByAny = map (ByAny . trimFilterId) excludeAny
      excludeByCategory = map (ByCategory . trimFilterId) excludeCategories
      excludeByTag = map (ByTag . trimFilterId) excludeTags
  -- combine all include criteria and all exclude criteria into a single FilterSpec
   in FilterSpec
        { fsIncludes = includeByAny ++ includeByCategory ++ includeByTag,
          fsExcludes = excludeByAny ++ excludeByCategory ++ excludeByTag,
          fsUseRegex = False
        }
