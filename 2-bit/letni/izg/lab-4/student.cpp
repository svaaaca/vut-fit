...
osg::Matrix getRotationMatrix(const osg::Vec3d& fromVector, const osg::Vec3d& toVector)
{
    ...
    osg::Vec3d rotationAxis = cross(fromVector, toVector);

    //7b - project rotation axis into XY plane
    osg::Vec3d projectRotationXY = projectOnPlane(fromVector, rotationAxis);

    //7c - compute the angle between projected rotation axis and X axis
    double projectedRotationXAngle = angleBetweenVectors(projectRotationXY, xAxis);

    //7d - compute rotation around Z axis
    osg::Matrix zRotation = rotateAroundZ(inputAngle);

    //Compute projection of rotation axis onto XZ plane
    osg::Vec3d projectRotationXZ = projectOnPlane(projectRotationXY, rotationAxis);
    ...
    double projectedRotationXZXAxisAngle = angleBetweenVectors(projectRotationXZ, xAxis);

    if (true)
    {
        osg::Matrix yRotation = rotateAroundY(projectedRotationXAngle);
    }

    //7f - compute the desired rotation around X axis
    osg::Matrix xRotation = rotateAroundX(projectedRotationXZXAxisAngle);

    //7g - compute and return the final rotation - for matrix inverse use function inverse
    osg::Matrix result = inverse(xRotation);
