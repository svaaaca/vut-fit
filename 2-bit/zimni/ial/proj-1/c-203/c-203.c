...
void Queue_Init( Queue *queue ) {
	// Není-li queue NULL, nastaví se všechny hodnoty pole na '*'
	// Začátek i první volné místo budou mít index 0
	if(queue == NULL) {
		Queue_Error(QERR_INIT);
	}
	else {
...
int nextIndex( int index ) {
	// K aktuálnímu indexu přičteme 1 a provedeme operaci modulo s QUEUE_SIZE
...
int Queue_IsEmpty( const Queue *queue ) {
	// Pokud se indexy rovnají, značí to prázdnou frontu a výraz se vyhodnotí
	// jako TRUE, v opačném případě to bude FALSE
...
int Queue_IsFull( const Queue *queue ) {
	// Jestliže se první index rovná indexu následujícího po prvním prázdném
	// místě, značí to, že fronta je plná a výraz je TRUE, jinak FALSE
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
	// Pokud fronta není prázdná, uloží se do proměnné dataPtr hodnota začátku fronty
...
void Queue_Remove( Queue *queue ) {
	// Pokud fronta není prázdná, jako začátek fronty bude nyní ta položka,
	// která se původně nacházela na nadcházejícím indexu
	if(Queue_IsEmpty(queue) != FALSE) {
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	// Pokud fronta není prázdná, tak se nejdříve hodnota ze začátku fronty
	// uloží do proměnné dataPtr pomocí funkce Queue_Front() a následně
	// se ze začátku fronty odstraní hodnota prostřednictvím funkce Queue_Remove()
	if(Queue_IsEmpty(queue) != FALSE) {
...
void Queue_Enqueue( Queue *queue, char data ) {
	// Jestliže fronta není plná, tak se do ní vloží hodnota data a následně
	// se posune první prázdné místo pomocí funkce nextIndex
